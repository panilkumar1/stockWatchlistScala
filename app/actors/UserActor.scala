...
class UserActor @Inject()(@Assisted out: ActorRef,
  ...
  override def receive: Receive = LoggingReceive {
    ...
    case StockHistory(symbol, price) =>
      val stockUpdateMessage = Json.obj("type" -> "stockhistory", "symbol" -> symbol, "price" -> price.doubleValue())
      out ! stockUpdateMessage
    ...
    case json: JsValue =>
      ...
      if (action == "add") {
        ...
        stocksActor ! UnwatchStock(Some(symbol))
      }
